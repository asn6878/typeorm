...
export class ReturningResultsEntityUpdator {
    ...
    async insert(
        ...
        if (!isReturningClauseSupported) {
            ...
            // for upsert queries without primary key, we need alternative ways to identify entities
            // split entities into two groups: those with IDs(pk defined by user) and those are not
            entities.forEach((entity, index) => {
                const entityId = metadata.getEntityIdMap(entity)
                if (!entityId) {
                    // this case means upsert queries ended with an update, and the given query doesn't have pk on it
